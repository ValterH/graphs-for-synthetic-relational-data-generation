...
def _add_nodes(metadata, digraph, names, details, node_colors = None):
    ...
    for table in metadata.get_tables():
        ...
        if node_colors is not None:
            if table in node_colors:
                digraph.node(table, label=title, fillcolor=node_colors[table])
            else:
...
def visualize_metadata(metadata, node_colors = None, path=None, names=True, details=True):
    ...
    _add_nodes(metadata, digraph, names, details, node_colors=node_colors)
